...
//			closestMass.remove();
	...
	private Mass findClosestMass(int mx, int my) {
		...
		for (Mass m: masses) {
			if (Math.pow(mx-m.getX(), 2) + Math.pow(my-m.getY(), 2) < minDist) {
				minDist = Math.pow(mx-m.getX(), 2) + Math.pow(my-m.getY(), 2);
				closest = m;
			}
		}
		return closest;
	...
	private void checkWallInflation() {
		if(getKey(KeyUp)){
			clearKey(KeyUp);
			WorldManager.getWorld().inflateWalls(Constants.PIXELS_FOR_INFLATION);
	...
	public void checkAssemblies() {
		if(getKey('C')){
			...
			clearKey('N');
			String file = JOptionPane.showInputDialog("Which assembly would you like to add?"
